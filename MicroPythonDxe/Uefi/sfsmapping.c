/** @file
  Consist mapping of Simple File System.
  ...
  )
{
  ...
  while (!IsDevicePathEnd (DevicePath) && DevicePathNodeLength(DevicePath) > 0) {
  ...
  )
{
  ...
  while (!IsDevicePathEnd (DevicePath)) {
    ...
    if (EFI_ERROR (Status)) {
      FREE_NON_NULL (MappingItem->Csd.Str);
      return Status;
    }
  ...
  )
{
  ...
  Str.Str = NULL;
  HIDevicePath = GetHIDevicePath ((EFI_DEVICE_PATH_PROTOCOL *)DevicePath);
  if (HIDevicePath == NULL) {
    goto Done;
  }
  ...
  Status = GetDeviceConsistMappingInfo (&MappingInfo, (EFI_DEVICE_PATH_PROTOCOL *)DevicePath);
  if (EFI_ERROR (Status)) {
    goto Done;
  }
